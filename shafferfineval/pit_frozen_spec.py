...
STILL_BLOCKED: dict[str, Any] = {
    "the_diagnostic_replay_itself": (
        ...
        "scorers and applies PEER_FLOOR_V1 and WITHIN_BLOCK_FLOOR_V1. What remains "
        "is a MEASUREMENT and an AUTHORISATION, not a specification: the 1.2 "
        "engine's memory is NOT YET MEASURED (Primitives grew; the share audits, "
        "the price reads and the TTM ladder entered the per-date cost -- a "
        "200-target smoke took 396 s, most of it the market side over 2,211 "
        "priced entities) and will be measured with pit_replay_rss one child at "
        "a time before any sizing pilot; the four-date pilot is rerun on a "
        "disposable DB only on the owner's word; the 165-date replay runs only on "
        "the owner's word. No replay authorisation exists."),
