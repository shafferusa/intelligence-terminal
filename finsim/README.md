...
| **Agency MBS / TBA** | Fannie Mae and Freddie Mac 30-year (UMBS), Ginnie Mae II 30-year and Fannie 15-year TBAs at a ladder of coupons, two delivery months each (FNCL-5.5-2026-10 …), settling on the SIFMA class A/B/C day: a forward, so cash moves at delivery and a TBA can be sold forward without a borrow. On delivery the line becomes the pool: monthly coupon, monthly principal paydowns from a prepayment model (refinancing S-curve on WAC − mortgage rate, seasoning, burnout; the mortgage rate is the 10-year plus 170bp so a rally speeds premiums up), pool factor. Effective duration and convexity from ±50bp shifts (premiums negatively convex), CPR, WAL, OAS, the dollar-roll drop. MBS repo at a 3% haircut. Listed proxies MBB, VMBS |
| **Structured credit** | 25 tranches: broadly syndicated and middle-market CLO debt (AAA to BB-, floating over the 3-month rate), conduit CMBS (A-4 to BBB-, fixed ten-year), prime-jumbo and non-QM RMBS, prime and subprime auto, credit-card and equipment ABS. Each tranche's spread (a discount margin for floaters) moves with the credit indices around its own norm and its rating migrates; a default writes down to recovery. Floaters carry rate duration to the next reset and spread duration on the fixed flows. Listed proxies JAAA, CLOA, CMBS |
| **Global government bonds** | Schatz / Bobl / Bund / Buxl, OATs, BTPs (EUR), Gilts (GBP), JGBs (JPY): each priced off its currency's curve (the dollar curve plus the policy-rate differential and a term shape; a country spread for France and Italy), settling T+2 / T+1 in its own currency. Bund, Bobl, Schatz, Buxl, Long Gilt and JGB futures margined in dollars with variation margin in the contract currency. Market Place → Bonds & Rates shows the EUR / GBP / JPY curves and the BTP−Bund and OAT−Bund spreads; `GET /worlds/{w}/yield-curve` carries every curve |
| **Multi-currency securities** | anything priced in another currency (foreign governments, native listings, EUR/GBP/JPY/HKD futures) is booked at its local price times spot: cost, marks, coupons, dividends and redemptions in the base currency, settlement cash in the local currency with the difference to the booked amount as realised FX. Pay with dollars on the ticket and the FX spot is dealt alongside |
| **Global index futures** | Euro Stoxx 50 (FESX, €10), DAX (FDAX, €25), FTSE 100 (FTSE, £10), Nikkei 225 in dollars (NKD), Hang Seng (HSIF, HK$50), MSCI Emerging Markets (MME) and MSCI EAFE (MFS): the cash index rides its ETF (FEZ, EWG, EWU, EWJ, EWH, EEM, EFA) in a simulated save and the real index (^STOXX50E, ^GDAXI, ^FTSE, ^N225, ^HSI) in a tracking one. Market Place → Indices & Vol |
| **Rates desk additions** | Ultra 10-Year (TN) and Ultra Bond (UB) futures; options on ZT/ZF/ZN/TN/ZB/UB, SOFR (SR3, strikes in eighths), ES/NQ and the Bund/Gilt/JGB contracts (Black-76, futures delivery, vol scaled to each contract); TIPS-5Y/10Y/30Y with a CPI index ratio, real yields and breakevens (nominal − real); the 2-year Treasury FRN at the 13-week bill + 15bp |
| **More commodities and currencies** | nickel, zinc, lead, tin (LME), iron ore (SGX), Dutch TTF gas and EU carbon allowances (in euros), JKM LNG; TRY, TWD, IDR, THB, CZK, HUF and the pegged SAR (NDFs for TWD and IDR) |
| **Currency loans** | borrow any quoted currency at its policy rate plus a spread by tier (G10 50bp … lira 300bp, more under stress) up to twice the NAV; interest accrues daily in the currency, the book retranslates the principal at spot, term loans repay themselves at maturity or roll, open loans reset daily; every cash balance earns its currency's policy rate. In a save that tracks the real market the policy rates are the real ones from FRED (the ECB deposit rate, the OECD immediate rates; the HKD and SAR pegs sit on the Fed's) and they also drive forwards and the local curves. Market Place → Foreign Exchange; `GET/POST /worlds/{w}/portfolios/{p}/ccy-loans`, `POST …/ccy-loans/{id}/repay` |
| **Native foreign listings** | 27 shares bought on their home exchange in their home currency: Tencent, Alibaba, AIA and China Mobile (HKD), Samsung, SK hynix and Hyundai (KRW), TSMC (TWD), Saudi Aramco and Al Rajhi (SAR), LVMH, L'Oréal, TotalEnergies, Airbus, SAP, Siemens, Allianz, ASML (EUR), Nestlé, Roche, Novartis (CHF), Toyota, Sony, MUFG (JPY), Reliance (INR), CBA and BHP (AUD). Ids are the Yahoo symbol with a dash (0700-HK). No listed options on these (the ADR has them). `python3 tools/universe_workbook.py` lists every one with its currency |
| **Treasury** | a new save opens with exactly one book, **Treasury**, holding all the starting capital; it backstops the other books: an order counts the Treasury's spare cash as available, a settlement or an overdraft (any currency) draws the shortfall from it automatically as an allocation (the default in the new-save dialog; the save's name is the world's). Every book you add draws its capital from the Treasury (the + Book dialog does it at creation; the Portfolio page and All Books have draw / return controls), returns what it can spare (settled cash after everything pending), and a deleted book's cash goes back to it. The Treasury is a book like any other (it can hold bills) but cannot be deleted; fresh capital is added to it. Older saves without one can create it (+ Book → Treasury). GET /worlds/{w}/treasury, POST /worlds/{w}/treasury/{fund,allocate,return} |
| **Universe workbook** | `python3 tools/universe_workbook.py --server http://127.0.0.1:8765 --world W-xxxx` writes one Excel file of everything a book can trade, one sheet per category: equities & funds (with options / lending / repo / TRS / perp flags), bonds, futures, listed-option underlyings and rules, currencies, commodities, the 28 OTC products, CDS reference names, financing & lending facilities, the playbook strategies and the simulated desks. Without `--server` it lists the shipped universe and the contract roots. Standard library only (`tools/xlsx_writer.py`) |
| **All Books** | the whole save as one big book, named after the world: every book's NAV, day / MTD / YTD / since-inception P&L, cash, long, short, net exposure, leverage and open items side by side with totals; positions added up across books with the split by book underneath; the day's P&L by bucket and cash by currency for all books together; the **Treasury balance sheet** (assets, liabilities and equity for the Treasury book and for all books added up, every line from the double-entry ledger and the closing marks, copy / CSV) with each book's draws from and returns to the Treasury against its NAV now. **Open Positions** with ★ ALL BOOKS selected is the blotter for the whole save: every fill, instruction, option strategy, OTC contract, loan, repo, FX spot (pending or settled), FX forward, private loan, private-equity company and a **Pending** table (working instructions, unsettled trades, pending FX, open RFQs, bids awaiting an answer) with the book on every line, the balance sheet at the end, one Copy everything / Download all (CSV). The book selector's first entry (★ ALL BOOKS) opens it; a flat book (nothing held, working or owed) can be deleted from here — never the last one, and its history stays in the event log |
...
| Live quotes (the live view, live tickets, live FX deals, the 15-minute quote updates) | the same Yahoo `spark` endpoint's metadata: regular-session price and time, up to 15 minutes delayed on most exchanges; stocks, ETFs, ADRs, REITs and coins directly; options off the underlying; futures off their source; bonds off the live Treasury curve (`^IRX ^FVX ^TNX ^TYX` shift the save's curve tenor by tenor and every bond is repriced, its spread unchanged); the dollar index and the VIX directly; commodity spots off the front-month continuous contract; cached one minute in memory, never written to disk | career saves: whenever a page or ticket asks, and every 15 minutes for instructions |
...
POST /api/worlds {..., treasury: true}   (the first and only book is the Treasury) · POST .../portfolios {..., from_treasury: true}   (a book funded from the Treasury) · POST .../portfolios {portfolio_type: TREASURY}   (make one in an older save)
