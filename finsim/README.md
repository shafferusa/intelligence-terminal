...
| **Treasury** | a new save opens with exactly one book, **Treasury**, holding all the starting capital (the default in the new-save dialog; the save's name is the world's). Every book you add draws its capital from the Treasury (the + Book dialog does it at creation; the Portfolio page and All Books have draw / return controls), returns what it can spare (settled cash after everything pending), and a deleted book's cash goes back to it. The Treasury is a book like any other (it can hold bills) but cannot be deleted; fresh capital is added to it. Older saves without one can create it (+ Book → Treasury). GET /worlds/{w}/treasury, POST /worlds/{w}/treasury/{fund,allocate,return} |
