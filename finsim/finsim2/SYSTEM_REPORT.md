...
| Learned Shaffer weights: find the weights history supports, then decide how much to trust them | DONE (research + live shadow) | `engine/learned.py`, `hedge/hedgelearn.py`, ML Lab → Learned weights / Learned hedge, `SHAFFER_LEARNED_WEIGHTS.md`: weights for all 74 signals at every node Global → Class → Product type → Sector → Industry → Asset and horizon; 10/10 planted-effect tests recovered; at 1W the global learned weights and the uniform-depth hierarchy beat production under every fixed gate + FDR (rank IC 0.053 / 0.055 vs 0.038) and are in live shadow; nothing significant 1M–12M; Directional never beats the prior; hedge sizing 1.35–1.5× supported but fails H4; production unchanged |
