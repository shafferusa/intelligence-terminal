...
(() => {
  ...
  const LAB_TABS = [['production', 'Production models'], ['performance', 'Historical performance'], ['signals', 'Signal research'], ['sigweights', 'Signal weights'], ['weights', 'Family weights'],
  ...
  const ciPct = a => a && a.acc != null ? `${fmt.pct(a.acc, 1)} <span class="faint">[${fmt.pct(a.lo, 1)}, ${fmt.pct(a.hi, 1)}]</span>` : '—';
  ...
  const labSignalWeights = (body, WR, vers) => {
    ...
    const main = order.map(lab => { const h = HS[lab], b = (h.challengers || {})[h.best] || {}, w = b.walkforward || {}, bl = w.baselines || {};
      ...
      return { lab, best: h.best, pa, ca, base: bl.best, ba: bl.best_acc, ex: ca != null && bl.best_acc != null ? ca - bl.best_acc : null, exp: pa != null && bl.best_acc != null ? pa - bl.best_acc : null, pic: (w.production || {}).ic, cic: (w.challenger || {}).ic, st: h.best ? statusOf(lab, h.best) : 'INSUFFICIENT DATA' }; });
    ...
    table($('#swMain'), main, [{ k: 'lab', label: 'Horizon', l: 1, f: x => `<b>${x.lab}</b><span class="sub">${esc(x.best || '')}</span>` }, { k: 'pa', label: 'Production accuracy', f: x => fmt.pct(x.pa, 1) }, { k: 'ca', label: 'Best challenger accuracy', f: x => fmt.pct(x.ca, 1) },
      { k: 'ba', label: 'Naive baseline', f: x => `${fmt.pct(x.ba, 1)}<span class="sub">${esc((x.base || '').replace(/_/g, ' '))}</span>` }, { k: 'ex', label: 'Excess vs baseline (challenger / prod.)', f: x => `<span class="${(x.ex || 0) > 0 ? 'pos' : 'neg'}">${fmt.spct(x.ex, 1)}</span> <span class="faint">/ ${fmt.spct(x.exp, 1)}</span>` },
      { k: 'pic', label: 'Production IC', f: x => fmt.num(x.pic, 3) }, { k: 'cic', label: 'Challenger IC', f: x => fmt.num(x.cic, 3) }, { k: 'st', label: 'Status', l: 1, f: x => statusPill(x.st) }], { sortKey: null });
    ...
    const bandCols = [{ k: 'band', label: 'Score', l: 1 }, { k: 'n', label: 'Records', f: b => fmt.num(b.n, 0) }, { k: 'independent', label: 'Indep. obs.', f: b => fmt.num(b.independent, 0) }, { k: 'positive', label: '% positive', f: b => fmt.pct(b.positive, 0) }, { k: 'mean', label: 'Avg return', f: b => fmt.spct(b.mean, 2) }, { k: 'median', label: 'Median', f: b => fmt.spct(b.median, 2) }, { k: 'correct', label: 'Direction right [95%]', f: b => b.correct != null ? `${fmt.pct(b.correct, 0)} <span class="faint">[${fmt.pct(b.lo, 0)}, ${fmt.pct(b.hi, 0)}]</span>` : '—' }];
    ...
    table($('#swW'), Object.entries(node.signals || {}).map(([s, v]) => ({ s, ...v, d: (v.challenger || 0) - (v.production || 0), st: stab[s] })), [{ k: 's', label: 'Signal', l: 1 }, { k: 'production', label: 'Production', f: x => fmt.spct(x.production, 1) }, { k: 'challenger', label: 'Challenger', f: x => `<b class="${scoreCls(x.challenger)}">${fmt.spct(x.challenger, 1)}</b>` },
      { k: 'd', label: 'Change', f: x => `<span class="${x.d > 0 ? 'pos' : 'neg'}">${fmt.spct(x.d, 1)}</span>` }, { k: 'st', label: 'Stable across eras?', l: 1, v: x => x.st && x.st.same_sign ? 1 : 0, f: x => x.st ? `${x.st.same_sign ? '<span class="pill pos" style="font-size:10px">same sign</span>' : '<span class="pill neg" style="font-size:10px">unstable</span>'} <span class="faint mono" style="font-size:11px">${x.st.eras.map(v => fmt.num(v, 2)).join(' · ')}</span>` : '<span class="faint">not used</span>' }], { sortKey: 'challenger', maxH: 520, empty: 'No weights for this node' });
  ...
  pages.ml = async (main, args, alive) => {
    ...
    if (tab === 'challengers') {
      ...
      table($('#chT'), vers, [{ k: 'id', label: 'Version', l: 1, f: v => `<b>${esc(v.id)}</b><span class="sub">${esc(v.description || '')}</span>` }, { k: 'kind', label: 'Engine', l: 1, f: v => v.kind === 'hedge' ? 'Shaffer Hedge' : 'Shaffer Score' },
