...
(() => {
  ...
  const LAB_TABS = [['production', 'Production models'], ['performance', 'Historical performance'], ['signals', 'Signal research'], ['alpha', 'Shaffer Alpha'], ['directional', 'Shaffer Directional'], ['sigweights', 'Signal weights'], ['weights', 'Family weights'],
  ...
  const labAlpha = (body, DR, vers) => {
    ...
    body.innerHTML = `<div class="card"><h2>Shaffer Alpha — does a higher score mean a better-than-normal outcome? <small>ranking, not absolute direction</small></h2>
      ...
      <div class="card flush" style="margin-top:14px"><h2>Signal weights: Alpha target vs Directional target <small>class-level challengers · share of the global node's |effective weight| · sign-stable = same sign in every era fit (CV = era-to-era variation)</small></h2><div id="alSg"></div></div>`;
  ...
  const labDirectional = (body, DR, vers) => {
    ...
    table($('#drMain'), order.map(k => { const b = (HS[k].directional || {})[HS[k].best_directional] || {}, w = b.walkforward || {}; const bear = (w.bearish || []).find(x => x.threshold === -20) || {}; return { k, br: (HS[k].wf_base_rate || {}).all, best: HS[k].best_directional, w, bear, st: (b.gates || {}).status, pa: HS[k].production_alpha || {} }; }),
      [{ k: 'k', label: 'Horizon', l: 1, f: x => `<b>${x.k}</b><span class="sub">${esc(x.best || '')}</span>` }, { k: 'br', label: 'Positive-return base rate', f: x => fmt.pct(x.br, 1) }, { k: 'aic', label: 'Alpha IC (rank)', f: x => `${fmt.num(x.pa.ic, 3)} <span class="faint">(${fmt.num(x.pa.rank_ic, 3)})</span>` },
        { k: 'acc', label: 'Directional accuracy', f: x => fmt.pct(x.w.accuracy, 1) }, { k: 'na', label: 'Naive accuracy', f: x => `${fmt.pct(x.w.baseline_acc, 1)}<span class="sub">${esc((x.w.baseline || '').replace(/_/g, ' '))}</span>` },
        { k: 'ex', label: 'Excess accuracy', f: x => `<span class="${(x.w.excess || 0) > 0 ? 'pos' : 'neg'}">${fmt.spct(x.w.excess, 1)}</span>` }, { k: 'bal', label: 'Balanced accuracy', f: x => fmt.pct(x.w.balanced_accuracy, 1) },
        { k: 'bs', label: 'Brier (skill)', f: x => `${fmt.num(x.w.brier, 4)} <span class="${(x.w.brier_skill || 0) > 0 ? 'pos' : 'neg'}">(${fmt.spct(x.w.brier_skill, 2)})</span>` }, { k: 'bp', label: 'Bearish precision', f: x => fmt.pct(x.w.bear_precision, 1) }, { k: 'st', label: 'Status', l: 1, f: x => statusPill(x.st) }], { sortKey: null });
