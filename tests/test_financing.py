...
class ShortSaleTest(unittest.TestCase):
    def test_short_sale_lifecycle(self):
        ...
        with self.assertRaises(CommandError):
            w.place_order(pf.id, "NVDA", "SELL", 1000)                    # nothing to deliver, no borrow
        loc = w.request_locate(pf.id, "NVDA", 100_000)
        ...
        o = w.place_order(pf.id, "NVDA", "SELL", loan.quantity)
        w.advance(1)
        ...
        self.assertEqual(led.security_balance("NVDA", "2600"), t.gross_amount)
        self.assertEqual(led.security_balance("NVDA", "1200"), t.net_amount, "proceeds are a receivable until settlement")
        ...
        self.assertEqual(led.security_balance("NVDA", "5300"), pos.borrow_fees)
        self.assertTrue(any(m.kind == "COLLATERAL" for m in pf.cash_movements))
        assert_ledger_invariants(self, w, pf)
        # cover and return
        o2 = w.place_order(pf.id, "NVDA", "BUY", loan.quantity)
        w.advance(1)
        ...
        self.assertEqual(led.security_balance("NVDA", "4000"), t2.realized_pnl)
        self.assertEqual(led.security_balance("NVDA", "2600"), D(0))
        self.assertEqual(led.security_balance("NVDA", "2610"), D(0))
        w.advance(2)
    ...
    def test_partial_and_failed_locate_and_expiry(self):
        ...
        big = w.request_locate(pf.id, "BYND", 50_000_000)          # far more than the lendable supply
        ...
        for _ in range(8):                                             # exhaust the lendable supply
            none = w.request_locate(pf.id, "BYND", 1_000_000)
            if none.status == "NONE":
                break
        ...
        loc = w.request_locate(pf.id, "JPM", 1000)
        w.advance(2)
        with self.assertRaises(CommandError):
    ...
    def test_borrow_fee_accrual_and_rerating(self):
        ...
        loc = w.request_locate(pf.id, "RIVN", 200_000)
        loan = w.borrow_securities(pf.id, loc.id, loc.available)
        w.place_order(pf.id, "RIVN", "SELL", loan.quantity)
        fees = []
        for _ in range(15):
            w.advance(1)
        ...
        self.assertAlmostEqual(loan.borrow_rate, w.market.lending.state["RIVN"].rate, places=6)
    ...
    def test_recall_replacement_borrow_and_buy_in(self):
        w, pf, _ = make_world(capital=200_000_000)
        loc = w.request_locate(pf.id, "LLY", 50_000)
        loan = w.borrow_securities(pf.id, loc.id, loc.available)
        w.place_order(pf.id, "LLY", "SELL", loan.quantity)
        w.advance(2)
        ...
        w.emit(E.SECURITY_LOAN_RECALLED, {"portfolio_id": pf.id, "loan_id": loan.id, "security_id": "LLY", "quantity": half, "due": due, "lender": loan.lender, "reason": "test recall"}, portfolio_id=pf.id)
        w.flush()
        ...
        loc2 = w.request_locate(pf.id, "LLY", half)
        ...
        self.assertEqual(pf.positions["LLY"].borrowed_quantity, loan.quantity + loan2.quantity)
        assert_ledger_invariants(self, w, pf)
        # a second recall left unanswered ends in a buy-in
        due2 = w.calendar.add_business_days(w.current_date, 2).isoformat()
        w.emit(E.SECURITY_LOAN_RECALLED, {"portfolio_id": pf.id, "loan_id": loan2.id, "security_id": "LLY", "quantity": loan2.quantity, "due": due2, "lender": loan2.lender, "reason": "test recall"}, portfolio_id=pf.id)
        w.flush()
        w.advance(3)
        ...
        self.assertEqual(pf.positions["LLY"].quantity, -loan.quantity)
        assert_ledger_invariants(self, w, pf)

    def test_collateral_marks_with_price(self):
        w, pf, _ = make_world(capital=300_000_000)
        loc = w.request_locate(pf.id, "AMD", 200_000)
        loan = w.borrow_securities(pf.id, loc.id, loc.available)
        w.place_order(pf.id, "AMD", "SELL", loan.quantity)
        for _ in range(10):
            w.advance(1)
            mv = loan.quantity * w.market.last_bar("AMD").close
    ...
    def test_treasury_collateral_for_borrow(self):
        ...
        w.advance(2)
        loc = w.request_locate(pf.id, "JPM", 100_000)
...
class CollateralTest(unittest.TestCase):
    def test_dashboard_and_double_pledge_prevention(self):
        ...
        w.place_order(pf.id, "NVDA", "BUY", 20_000)
        w.advance(2)
...
class PrimeTest(unittest.TestCase):
    def test_margin_borrowing_interest_and_repayment(self):
        ...
        led = w.ledgers[pf.id]
        w.place_order(pf.id, "SPY", "BUY", 60_000)
        w.advance(2)
        ...
        w.place_order(pf.id, "LLY", "BUY", 40_000)                # spend the drawn cash so the loan persists
        ...
        w.place_order(pf.id, "LLY", "SELL", 40_000)
        w.place_order(pf.id, "NVDA", "SELL", 100_000)
        w.advance(2)
        ...
        px = w.market.last_bar("SPY").ask
        w.place_order(pf.id, "SPY", "BUY", int(cash / px * D("1.3")))
        w.advance(2)
    ...
    def test_overdraft_becomes_loan_never_negative_cash(self):
        ...
        w.place_order(pf.id, "SPY", "BUY", 100_000)
        w.advance(2)
        loc = w.request_locate(pf.id, "NVDA", 60_000)
        ...
        w.place_order(pf.id, "NVDA", "SELL", loan.quantity)
        for _ in range(6):
            w.advance(1)
...
class DefinitionOfDoneTest(unittest.TestCase):
    ...
    def test_scenario(self):
        ...
        self.assertGreater(loc.available, 0)
        loan = w.borrow_securities(pf.id, loc.id, min(loc.available, D(int(20_000_000 / float(w.market.last_bar(tkr).close)))))   # ~$20m short, whatever the name costs
