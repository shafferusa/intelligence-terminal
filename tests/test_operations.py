...
class CorporateEventsTest(unittest.TestCase):
    def test_stock_and_special_dividends(self):
        ...
        w.place_order(pf.id, "BRWN", "BUY", 10_000)
        w.advance(2)
        ev = announce(w, "BRWN", "STOCK_DIVIDEND", {"ratio": 0.10}, 2)
        ca = w.corporate_actions[ev.payload["id"]]
        self.assertEqual(ca.action_type, "STOCK_DIVIDEND")
        nav0 = w.ledgers[pf.id].nav()
        w.advance(2)
        self.assertEqual(pf.positions["BRWN"].quantity, D(11_000), "10% stock dividend adds shares")
        self.assertEqual(ca.status, "PAID")
        assert_ledger_invariants(self, w, pf)
        ev2 = announce(w, "BRWN", "SPECIAL_DIVIDEND", {"amount": 2.50}, 2)
        w.advance(2)
        div = [c for c in w.corporate_actions.values() if c.security_id == "BRWN" and c.action_type == "CASH_DIVIDEND" and c.id.endswith("SPECIAL")]
        self.assertEqual(len(div), 1)
        self.assertEqual(div[0].entitlements[pf.id]["amount"], D("27500.00"))
        self.assertEqual(w.ledgers[pf.id].security_balance("BRWN", "1210"), D("27500.00"), "special dividend receivable")
        w.advance(10)
    ...
    def test_tender_offer_election_and_settlement(self):
        w, pf, store = make_world(capital=20_000_000)
        w.place_order(pf.id, "ORCH", "BUY", 10_000)
        w.advance(2)
        ev = announce(w, "ORCH", "TENDER_OFFER", {"offer_price": 250.0, "max_pct": 0.25, "deadline": w.calendar.add_business_days(w.current_date, 2).isoformat(), "bidder": "Atlas Capital Group"}, 4)
        ca_id = ev.payload["id"]
        ...
        w.advance(3)
        self.assertEqual(pf.positions["ORCH"].quantity, D(8_000))
        t = [pf.trades[t] for t in pf.positions["ORCH"].trade_ids if pf.trades[t].side == "SELL"][0]
    ...
    def test_rights_issue_subscription(self):
        w, pf, store = make_world(capital=20_000_000)
        w.place_order(pf.id, "GRDP", "BUY", 20_000)
        w.advance(2)
        px = float(w.market.last_bar("GRDP").close)
        ev = announce(w, "GRDP", "RIGHTS_ISSUE", {"subscription_price": round(px * 0.8, 2), "ratio": 0.25, "deadline": w.calendar.add_business_days(w.current_date, 1).isoformat()}, 3)
        with self.assertRaises(CommandError):
        ...
        w.advance(3)
        self.assertEqual(pf.positions["GRDP"].quantity, D(25_000))
        t = [pf.trades[t] for t in pf.positions["GRDP"].trade_ids if pf.trades[t].execution_detail.get("contractual")][0]
    ...
    def test_cash_merger_cashes_out_holders_settles_options_and_delists(self):
        ...
        ch = w.options.chain("TITN", w.options.chain("TITN")["expiries"][2])
        atm = min(ch["rows"], key=lambda r: abs(r["strike"] - ch["level"]))
        w.place_order(pf.id, "TITN", "BUY", 5_000)
        w.place_order(pf.id, atm["C"]["id"], "BUY", 10)
        w.advance(2)
        px = float(w.market.last_bar("TITN").close)
        deal = round(px * 1.3, 2)
        ev = announce(w, "TITN", "CASH_MERGER", {"deal_price": deal, "acquirer": "Atlas Capital Group"}, 3)
        w.advance(1)
        self.assertGreater(float(w.market.last_bar("TITN").close), px * 1.1, "the stock jumps toward the deal price on the announcement")
        w.advance(2)
        self.assertEqual(pf.positions["TITN"].quantity, D(0))
        t = [pf.trades[t] for t in pf.positions["TITN"].trade_ids if pf.trades[t].side == "SELL"][0]
        ...
        self.assertTrue(w.securities["TITN"].delisted)
        with self.assertRaises(CommandError):
            w.place_order(pf.id, "TITN", "BUY", 100)
        w.advance(2)
        self.assertEqual(pf.positions["TITN"].settled_quantity, D(0))
        assert_ledger_invariants(self, w, pf)
        w2 = World.load(store, "t")
        self.assertTrue(w2.securities["TITN"].delisted)
        self.assertEqual(w2.ledgers[pf.id].nav(), w.ledgers[pf.id].nav())

    def test_spin_off_creates_a_new_security(self):
        w, pf, store = make_world(capital=30_000_000)
        w.place_order(pf.id, "AERO", "BUY", 4_000)
        w.advance(2)
        cost0 = pf.positions["AERO"].cost_basis
        ev = announce(w, "AERO", "SPIN_OFF", {"fraction": 0.2, "ratio": 0.5, "new_id": "AERS", "new_name": "Aerion Spinco", "sector": "Industrials"}, 3)
        ...
        self.assertEqual(pf.positions["AERS"].cost_basis + pf.positions["AERO"].cost_basis, cost0, "cost basis is carved out, not created")
        assert_ledger_invariants(self, w, pf)
        w.advance(1)
    ...
    def test_bond_call_redeems_at_the_call_price(self):
        ...
        w.advance(2)
        held = pf.positions["NGSL-30"].quantity
        self.assertEqual(held, D(1_000_000))
        ev = announce(w, "NGSL-30", "BOND_CALL", {"call_price": 102.0}, 3)
        w.advance(3)
        ...
        self.assertTrue(w.securities["NGSL-30"].delisted)
        assert_ledger_invariants(self, w, pf)
...
class FailChargesTest(unittest.TestCase):
    def test_failed_delivery_is_charged_and_bought_in(self):
        ...
        ch = w.options.chain("MRDN", w.options.chain("MRDN")["expiries"][0])
        ...
        w.advance(1)
        days = 0
        while not any(si.status == "FAILED" for si in pf.settlements.values() if si.instruction_type == "DVP" and si.security_id == "MRDN") and days < 30:
            w.advance(1)
            days += 1
        fails = [si for si in pf.settlements.values() if si.instruction_type == "DVP" and si.security_id == "MRDN" and si.status == "FAILED"]
